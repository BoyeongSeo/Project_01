	...
	: TForm(Owner)
{
 ...
 BigQueryRowCount=0;
 BigQueryFileCount=0;
 printf("check log integrity\n");
 logger.check_log_integrity();
...
{
  logger.info("PurgeButtonClick: Purging stale aircraft data");
  uint32_t *Key;
  ght_iterator_t iterator;
  TADS_B_Aircraft* Data;
  void *p;
 ...
 // Form1->MsgLog->Lines->Add(StringMsgBuffer);
  if (Form1->RecordRawStream)
  {
   ...
   CurrentTime=GetCurrentTimeInMsec();
	...
	String original = IntToStr(CurrentTime);
	std::string plaintext = AnsiString(original).c_str();

	std::string iv_hex, tag_hex, cipher_hex;
	if (encrypt_aes256_gcm(plaintext, iv_hex, tag_hex, cipher_hex)) {
		String encoded_line = String(iv_hex.c_str()) + "|" + 
		String(tag_hex.c_str()) + "|" +
		String(cipher_hex.c_str());
		Form1->RecordRawStream->WriteLine(encoded_line);
	}
	//    Form1->RecordRawStream->WriteLine(StringMsgBuffer);
	plaintext = AnsiString(StringMsgBuffer).c_str();
	if (encrypt_aes256_gcm(plaintext, iv_hex, tag_hex, cipher_hex)) {
		String encoded_line = String(iv_hex.c_str()) + "|" + 
							String(tag_hex.c_str()) + "|" +
							String(cipher_hex.c_str());
...
//  logger.info(StringMsgBuffer.c_str());
...
void __fastcall TForm1::RawConnectButtonClick(TObject *Sender)
{
 ...
 {
    std::string host = UTF8Encode(IdTCPClientRaw->Host).c_str();
    std::string port = AnsiString(IntToStr(IdTCPClientRaw->Port)).c_str();
    std::string msg = "RawConnectButtonClick connect: " + host + ":" + port;
  	logger.info(msg);
  try
   {
   ...
   TIdSSLIOHandlerSocketOpenSSL *sslHandler = new TIdSSLIOHandlerSocketOpenSSL(nullptr); // �����ڴ� ���� ���̳� this
   sslHandler->SSLOptions->Method = sslvTLSv1_2;  // version limitation

   sslHandler->SSLOptions->CipherList = "ECDHE-RSA-AES128-GCM-SHA256:ECDHE-RSA-AES256-GCM-SHA384:AES128-SHA";
   ...
   sslHandler->SSLOptions->Mode = sslmClient; //  Client mode, sslmUnassigned = auto
   String certPath = TPath::Combine(TPath::GetDirectoryName(Application->ExeName), "cert");
   sslHandler->SSLOptions->RootCertFile = TPath::Combine(certPath, "pi-ca.crt");

   sslHandler->SSLOptions->VerifyMode = TIdSSLVerifyModeSet() << sslvrfPeer; // "must verify peer" setting

   // TIdSSLVerifyModeSet verifyModes;
   // verifyModes << sslvrfPeer << sslvrfFailIfNoPeerCert;
   //sslHandler->SSLOptions->VerifyMode = verifyModes;
   // sslHandler->SSLOptions->VerifyDepth = 2; // cert chain limit. we are using server-ca.crt as a root ca. so 1 would be enough

   //sslHandler->SSLOptions->CipherList = "ECDHE-RSA-AES256-GCM-SHA256";
 ...
 {  // disconnect
    std::string host = UTF8Encode(IdTCPClientRaw->Host).c_str();
    std::string port = AnsiString(IntToStr(IdTCPClientRaw->Port)).c_str();
    std::string msg = "RawConnectButtonClick disconnect: " + host + ":" + port;
  	logger.info(msg);
...
{
  	logger.info("IdTCPClientRawConnected");
   ...
   // Form1->IdTCPClientRaw->ReadTimeout = 10000; // set timeout 10s
...
{
  logger.info("IdTCPClientRawDisconnected");
  TCPClientRawHandleThread->Terminate();
}
//---------------------------------------------------------------------------
void __fastcall TForm1::RawRecordButtonClick(TObject *Sender)
{
 ...
 {
  logger.info("RawRecordButtonClick Record ");
	...
	if (RecordRawStream==NULL)
	  {
		ShowMessage("Cannot Open File "+RecordRawSaveDialog->FileName);
		 ...
		 std::string fileName = UTF8Encode(RecordRawSaveDialog->FileName).c_str();
		 std::string msg = "RawRecordButtonClick Record File: " + fileName;
		 logger.info(msg);
 ...
 {
   logger.info("RawRecordButtonClick Stop");
...
void __fastcall TForm1::RawPlaybackButtonClick(TObject *Sender)
{
 ...
 {
	logger.info("RawPlaybackButtonClick Playback");
	...
	if (PlayBackRawStream==NULL)
	  {
		ShowMessage("Cannot Open File "+PlaybackRawDialog->FileName);
		   ...
		   std::string fileName = UTF8Encode(PlaybackRawDialog->FileName).c_str();
		   std::string msg = "RawPlaybackButtonClick Playback File: " + fileName;
		   logger.info(msg);
		  }
	}
  }
 ...
 {
	std::string msg = "RawPlaybackButtonClick Stop: ";
	logger.info(msg);
   TCPClientRawHandleThread->Terminate();
	  ...
	  try
        {
		 ...
		 StringMsgBuffer = DecryptEncryptedLine(StringMsgBuffer);
...
void __fastcall TForm1::SBSConnectButtonClick(TObject *Sender)
{
 ...
 if ((SBSConnectButton->Caption=="SBS Connect") && (Sender!=NULL))
 {
  try
   {
    std::string host = UTF8Encode(IdTCPClientSBS->Host).c_str();
    std::string port = AnsiString(IntToStr(IdTCPClientSBS->Port)).c_str();
    std::string msg = "SBSConnectButtonClick connect: " + host + ":" + port;
  	logger.info(msg);
 ...
 else
  {
    std::string host = UTF8Encode(IdTCPClientSBS->Host).c_str();
    std::string port = AnsiString(IntToStr(IdTCPClientSBS->Port)).c_str();
    std::string msg = "SBSConnectButtonClick disconnect: " + host + ":" + port;
  	logger.info(msg);
 ...
 // Form1->MsgLog->Lines->Add(StringMsgBuffer);
  if (Form1->RecordSBSStream)
  {
   ...
   if (encrypt_aes256_gcm(plaintext, iv_hex, tag_hex, cipher_hex)) {
	   ...
	   String(tag_hex.c_str()) + "|" +
	   String(cipher_hex.c_str());
	   Form1->RecordSBSStream->WriteLine(encoded_line);
	}
...
void __fastcall TForm1::SBSRecordButtonClick(TObject *Sender)
{
 ...
 {
	logger.info("SBSRecordButtonClick Record");
	...
	if (RecordSBSStream==NULL)
	  {
		...
		std::string fileName = UTF8Encode(RecordSBSSaveDialog->FileName).c_str();
		std::string msg = "SBSRecordButtonClick Record File: " + fileName;
		logger.info(msg);
 ...
 {
   std::string msg = "SBSRecordButtonClick Stop: ";
   logger.info("SBSRecordButtonClick Stop");
...
void __fastcall TForm1::SBSPlaybackButtonClick(TObject *Sender)
{
 ...
 {
	logger.info("SBSPlaybackButtonClick Playback");
	...
	if (PlayBackSBSStream==NULL)
	  {
		ShowMessage("Cannot Open File "+PlaybackSBSDialog->FileName);
		   ...
		   std::string fileName = UTF8Encode(PlaybackSBSDialog->FileName).c_str();
		   std::string msg = "SBSPlaybackButtonClick Playback File: " + fileName;
		   logger.info(msg);
		  }
	}
  }
 ...
 else
 {
   ...
   logger.info("SBSPlaybackButtonClick Stop");
 }
...
{
   logger.info("IdTCPClientSBSConnected");
   //SetKeepAliveValues(const AEnabled: Boolean; const ATimeMS, AInterval: Integer);
...
{
  logger.info("IdTCPClientSBSDisconnected");
  TCPClientSBSHandleThread->Terminate();
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::UseSBSRemoteClick(TObject *Sender)
{
 ...
 logger.info("UseSBSRemoteClick: Using ADS-B Hub server");
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::UseSBSLocalClick(TObject *Sender)
{
 ...
 logger.info("UseSBSLocalClick: Using ADS-B Local server");
}
//---------------------------------------------------------------------------
static bool DeleteFilesWithExtension(AnsiString dirPath, AnsiString extension)
 {
...
{
    TPasswordBoxForm *PwdForm = new TPasswordBoxForm(this);
